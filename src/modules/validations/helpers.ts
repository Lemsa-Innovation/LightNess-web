...
export type ErrorIds = keyof (typeof FieldValidationMessages & typeof NumberValidationMessages)

const getNumberMessages = ({ errorMessage, rules }: {
    errorMessage: string
    rules?: Rules
}) => {
    switch (errorMessage as keyof typeof NumberValidationMessages) {
        ...
        default: {
            if (errorMessage.includes("numberMax")) {
                ...
                if (maxValue) {
                    return rules?.number.maxValue.replace('{maxValue}', maxValue)
                }
            } else if (errorMessage.includes("numberMin")) {
                const minValue = errorMessage.match(/\d+/)?.[0]; // Extraire la valeur minimale
                if (minValue) {
                    return rules?.number.minValue.replace('{minValue}', minValue)
                }
            }
        }
    }
...
const getRuleErrors = ({ errorMessage, rules }: {
    errorMessage: string
    rules?: Rules
}) => {
    ...
    switch (errorMessage as ErrorIds) {
        ...
        default: {
            if (errorMessage.includes("fieldTooLong")) {
                ...
                if (maxLength) {
                    return rules?.field.maxLength.replace('{maxLength}', maxLength)
                }
            } else if (errorMessage.includes("fieldTooShort")) {
                const minLength = errorMessage.match(/\d+/)?.[0]; // Extraire la valeur maximale
                if (minLength) {
                    return rules?.field.minLength.replace('{minLength}', minLength)
                }
            }
            return errorMessage
        }
